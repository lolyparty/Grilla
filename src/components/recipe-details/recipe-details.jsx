...
const RecipeDetails = ({likedItems, setLikedItems, setShowLikes})=>{
    ...
    const Ingredients = (initial,number) =>{
        const data = ingredientsDetails.slice(initial,number).map((e, index)=>{
            return <p className="my-6 xxsm:my-3 text-xl xxsm:text-lg" key={index}><img className="inline-block mr-1" src={point} alt='ingredient' />{e.quantity}{e.unit} {e.description} </p>
        })
        return data
    }
    ...
    return <div>
                ...
                <div className="flex justify-center z-50">
                       {detailsResult.title ?  <p className="p-5 transform -translate-y-10 details_name text-white font-bold text-2xl xxsm:text-sm xxsm:p-2 xxsm:-translate-y-5">{detailsResult.title}</p> : null}
                </div>
                <div className="mx-auto my-2 flex justify-between w-10/12 items-center" >
                    <div className="text-ingredientColor font-bold text-xl flex pt-2.5 items-center">
                ...
                <div className="more text-center bg-gray-200 p-5 pb-9">
                    ...
                    <div>
                    <a href={detailsResult.source_url} className="p-3 px-5 bg-background2 my-10 xxsm:my-5 xxsm:mt-3 text-white rounded-xl mx-auto font-bold text-xl xxsm:text-lg xxsm:px-3 xxsm:p-2" rel="noreferrer" target="_blank"> Directions <img src={arrow} alt='directions' className="inline-block mb-0.5"/></a>
                    </div>
                    <div className="share">
                    <p className="mt-7 text-xl xxsm:text-lg text-ingredientColor inline-block">Share this recipe with family and friends: </p>
